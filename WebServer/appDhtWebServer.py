...
def getLastData():
        ...
        for row in curs.execute("SELECT * FROM lux_data ORDER BY timestamp DESC LIMIT 1"):
                time = str(row[0])
                lux = row[1]
                
    #conn.close()
        return time, temp, hum , distance , lux
...
def getHistData2 (numSamples):
        conn=sqlite3.connect('../sensorsData.db')
        curs=conn.cursor()
        curs.execute("SELECT * FROM lux_data ORDER BY timestamp DESC LIMIT "+str(numSamples))
        data2 = curs.fetchall()
        dates2 = []
        luxes = []
...
def testeData2(luxes):
    n = len(luxes)
    for i in range(0, n-1):
        if (luxes[i] < 0 or luxes[i] >4000):
            luxes[i] = luxes[i-2]
    return luxes
...
@app.route("/")
def index():
    time, temp, hum, distance, lux = getLastData()
    templateData = {
      'time'        : time,
      'temp'        : temp,
      'hum'         : hum,
...
@app.route('/plot/lux')
def plot_lux():
    times, luxes = getHistData2(numSamples)
    ys = luxes
    print(luxes)
    fig = Figure()
    axis = fig.add_subplot(1, 1, 1)
    axis.set_title("Illuminnance[Lux]")
    axis.set_xlabel("Time")
    axis.grid(True)
